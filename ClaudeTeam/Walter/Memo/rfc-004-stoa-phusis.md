...
**Subscriber 전달 분류 — `polling-only` vs `push-deliverable`** (사이클 10 Stoa#13 트랙 A/B 신설, `8d91eea` 코드 + 본 patch spec):

envelope `address`는 *identity canonical URL* (룰 25 doctrine — `https://<stoa_host>/inbox/<name>`). 동시에 `_pump_subscriber`가 이를 HTTP push target으로 해석해버리면 *자기 host에 POST → 404 → urllib socket alloc 누적 → RSS leak*(Stoa#11/#12 self-loop). 즉 한 string이 두 의미(*누구인지* / *어디로 보낼지*)를 동시에 표현하려다 자기 충돌하는 자리.

해소 doctrine:

1. **`polling-only` subscriber** — 자체 HTTP listener 없음, Stoa GET `/api/v1/messages?to=<name>` 또는 `/api/v1/inbox?to=<name>`(§4) 폴링으로 letter 회수. `address`는 identity canonical 그대로. Stoa는 *push 0*, 모든 pending letter `status="skipped"` 마크. ClaudeTeam·CAST 멤버·AIL 5인(homeros·arche·ergon·telos·tekton) 모두 본 분류.
2. **`push-deliverable` subscriber** — 외부 HTTP ingestion endpoint 보유. Stoa가 envelope를 그 endpoint로 POST. 이 경우 `address` (identity canonical)와 *별도로* delivery target이 표현돼야 함 — 후보:
   - (a) 별 `delivery_endpoint: Text` Optional 필드를 registry/`subscriber.<name>` Record에 추가.
   - (b) 별 도메인 prefix를 가진 address (예: `https://hook.example.com/stoa-in/<name>`)로 등록해 *identity URL이 곧 push target*인 케이스 한정.

Phase 1+(룰 25 land 이후) **default 분류는 `polling-only`** — Stoa registry address는 정의상 identity canonical이라 자기 host로 떨어진다. push-deliverable이 필요한 첫 외부 ingestion 등록 시점에 (a)/(b) 결정.

**판정 알고리듬** (`8d91eea` 자취):

```
_is_polling_inbox(name, addr) := addr endswith "/inbox/<name>"
```

suffix-only invariant — `_is_self_host(addr, self_origin)`보다 *강한 doctrine-level* 판정. autonomous tick context에서 `server.self_origin` latch 결손이어도 안전 (Stoa#13 트랙 A 학습). `_pump_subscriber` 진입 직후 `_is_polling_inbox` 게이트 통과 시 즉시 `status="skipped"` 일괄 마크 + push 시도 0.

**향후 (a) 구현 시** `subscriber.<name>.delivery_endpoint != ""`면 그 endpoint로 push, `""`면 polling-only. 본 RFC는 결정 자리만 박고 구현은 첫 push-deliverable 등록 트리거에 자연 fold (§13 q-subscriber-classify).
...
**런타임 환경**: `schedule.every`는 *long-running runtime*에서 작동 — `ail up` 또는 `ail run + evolve`(server 진입) 둘 다. 사이클 8 (2026-05-08) AIL Telos runtime 정정 정합 (arche 권고 + 박상현 GO `msg_1778191592_34`). 새 primitive 0, grammar 동일, *적용 자리 확장*만 — 본 RFC §3.1 코드 형태 변경 0, 문장 보강 한 줄로 정합.
...
- subscriber `<name>`에 미전달 letter 있고 분류가 *push-deliverable*이면(§2.2) → **deliver** action 후보.
- subscriber 분류가 *polling-only* (`address` suffix == `/inbox/<name>`, 룰 25 default) → **skip** (counter++) 후보. 클라이언트가 자기 polling으로 회수.
- self-host address (issue#3 doctrine, `_is_self_host` self_origin 매칭) → **skip** (counter++) 후보. polling-only 판정의 보강 가드 (latch 결손 fallback).
...
- `schedule.every(TICK_SEC)` 등록 (`on_genesis`). 런타임 환경: `ail up` 또는 `ail run + evolve` (§3.1 정합, 사이클 8 AIL Telos runtime 정정 후).
...
**Self-loop 가드 (사이클 10 Stoa#13 학습)**: `_pump_subscriber`는 진입 즉시 §2.2 subscriber 분류 게이트 통과 — polling-only 분류면 push 0 + `status="skipped"` 일괄 마크 후 return. 룰 25 envelope address doctrine과 정합. `8d91eea` 코드 자취. `server.self_origin` latch가 autonomous tick context에서 결손되는 정황에도 안전 — suffix invariant가 latch보다 강한 doctrine-level 가드.
...
| 요구 | AIL primitive | 충족 |
|---|---|---|
| 자율 loop | `schedule.every(N)` + `entry main` | ✓ (long-running runtime: `ail up` 또는 `ail run + evolve`, 사이클 8 AIL Telos 정정 정합) |
...
- **q-subscriber-classify** (v1.7) — push-deliverable subscriber 첫 외부 ingestion 등록 시점에 (a) `subscriber.<name>.delivery_endpoint` Optional 필드 추가 / (b) 별 도메인 prefix address 결정. 그때까지 default `polling-only` (룰 25 envelope address doctrine 직접 함의). 본 RFC는 결정 자리만 박고 구현은 트리거 발생 사이클에 자연 fold.
...
- v1.6 (2026-05-08) — Phase B β-modified 정합: §3.1·§6.2·§11.1 phrasing 정정 (arche 권고 + 박상현 GO `msg_1778191592_34`). `schedule.every`는 *long-running runtime*에서 작동 — `ail up` 또는 `ail run + evolve` 둘 다. AIL Telos가 `ail run + evolve` 환경에서 schedule.every 작동하도록 runtime 인공 제약 해제 — 새 primitive 0, grammar 동일, *적용 자리 확장*만. RFC 코드 형태 변경 0, 문장 보강.
- v1.7 (2026-05-15) — Stoa#13 self-loop leak 정합 (사이클 10 트랙 B): §2.2에 *Subscriber 전달 분류* (`polling-only` vs `push-deliverable`) 박음 — envelope `address`는 identity canonical URL(룰 25)이고, HTTP push deliverability는 `_is_polling_inbox(name, addr) := addr endswith "/inbox/<name>"` suffix invariant로 판정. Stoa#13 트랙 A 코드(`8d91eea`) 자취. §3.3 Reason 후보 분류 명시(*push-deliverable*만 deliver 후보, *polling-only*는 skip 후보). §6.2 Phase B 자기 self-loop 가드 명시 — `_pump_subscriber` 진입 즉시 polling-only 게이트 통과 시 push 0 + skipped 일괄 마크. autonomous tick context에서 `server.self_origin` latch 결손에도 안전(suffix invariant가 latch보다 강한 doctrine-level 가드).
