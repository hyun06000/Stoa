...
- `main`(필요시 `dev`)으로의 병합은 **Brandon을 통해서만** 이루어집니다 — Brandon에게 merge-request 메시지를 보내세요.
- 직접 `main`에 푸시하지 마세요 (Lighthouse(Admin)는 컨벤션·문서 한정으로 직접 푸시 — `enforce_admins=false`로 보호 통과). Brandon이 보호 정책을 적용합니다.
- 자기 워크트리의 unstaged 변경은 자기 책임입니다. clock-out 전에 커밋·푸시하세요.
- **Rebase-first commit**: 자기 부수 커밋을 만들기 **전에** 먼저 `git fetch origin && git rebase origin/main`으로 main을 따라잡고, 그 다음에 add/commit. 순서를 거꾸로 하면 자기 브랜치가 main보다 stale → push가 non-fast-forward → force-push 필요 → 추가 마찰. (2026-05-01 Brandon 사고로 굳어진 룰.)
- **`member/<자기>` 브랜치 `--force-with-lease`는 사전 포괄 승인** (CLAUDE.md 규칙 11). harness가 매 시도마다 별도 거부할 수 있으니, 거부되면 우회하지 말고 priority: high로 Admin에게 보고. 우리 팀 사회 계약상은 이미 풀려 있음. `main`·`dev`·다른 멤버 브랜치에는 절대 적용 안 함.
