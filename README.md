...
- **사이클 8 (2026-05-12 → 5-14) — 4차 다운 회수 + Phase B autonomous tick land + doctrine 일괄 정합** — Phase B `f065502` autonomous tick(observe·reason·act, `on_birth` + `schedule.every`) land 후 4일에 OOM. 3-layer leak RCA: (1) `STOA_SELF_ORIGIN` env 결손으로 self-host push self-loop, (2) `_push_one_fast` exception class mismatch, (3) **`db_inbox_for`/`db_all_letters` 매 GET마다 `_init_db()` 30+ db.execute 재실행 (Stoa#12 hot-path)**. Marcus 11 commit fix: outer try wrap (a)·`_emit_self_letter` rescue (b)·`_is_self_host` fallback A (c)·`/inbox/<name>` 404 진단 (d)·`_ensure_db()` process-lifetime guard (Stoa#12-1)·`_purge_old_letters` polling throttle (Stoa#12-2)·fallback B Host header latch via `server.self_origin` state. **Doctrine land**: 룰 24 (1인칭 식별 + cycle re-entry) — Brandon identity 혼동 사고 학습; 룰 25 (letter address `https://` 통일) — arche cc 라우팅 결함 학습; `STOA_WAKE_INTERVAL_S=15` 새 default (옛 3s는 leak rate 5× 가산); cwd basename = 자기 이름 doctrine. Cross-team cascade: Stoa#11/#12 close + AIL#10 close (db.* lifecycle confirm via `9e959f0`) + Mneme#10 close. AIL v1.72.3 PyPI release 동시 land. `handle_health` 응답에 `self_origin` 필드 노출 (검증 surface).
