...
### §11 미리 적은 핵심 발견 (사전 학습 결과)
- `crypto_sign_ed25519` / `crypto_keygen_ed25519` AIL stdlib에 **부재** (reference card 1.8 line 414 직접 확인). verify만 있다.
- Cryptographic random 또한 stdlib에 없음 (line 414 주변 `random`/`uuid` 검색 결과 0건).
- 비대칭이 의미하는 바: **서명·키 생성·nonce 모두 AIL 외부에서 처리하고 hex/base64로 들여와야 한다.** 또는 AIL upstream에 추가 요청 (Cross-repo workflow, CLAUDE.md `46058f8`).
- 본 결정은 §11에서 옵션·근거로 분리. 지금 단정 안 함.

### §13 미리 적은 미결 (mid-review 피드백 반영)
- `human accounts` — split to RFC-002.
- `content safety / PII / secret filter` — split to RFC-003.
- `predictable nonce / weak random` — 발신자 측 cryptographic random 책임. Stoa는 검증 불가.
- `time-skew abuse` — created_at window 운영 튜닝. v1 ±60s 기본.
- `post-compromise recovery` — compromised LA 시 사용자 admin이 registry에 'revoke' row를 직접 INSERT하는 메커니즘 등. 본 RFC에서는 actor 승격 안 함; 후속 RFC 또는 운영 매뉴얼.
