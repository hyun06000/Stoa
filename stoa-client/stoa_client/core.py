...
class KeyExistsError(FileExistsError):
    """Raised by keygen when a private key file already exists and `force`
    was not requested. Prevents accidental overwrite of an in-use identity."""
...
def keygen(
    ...
    `force` is False — overwrite would silently invalidate every signature
    previously made under this identity.
    """
    ...
    if sk_path.exists() and not force:
        raise KeyExistsError(
            f"private key already exists: {sk_path}. "
            f"Pass force=True (or `--force`) to overwrite — "
            f"this invalidates every prior signature under '{identity}'."
        )
