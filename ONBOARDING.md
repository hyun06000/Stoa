...
3. **워크트리 보유 멤버는 자기 워크트리로 작업 공간 변경**: `cd <repo>/../<자신>/` (즉 `Stoa/<자신>/`). 이후 모든 작업(identity 읽기, Memo 훑기, inbox 점검, monitor 가동, commit)은 **워크트리 안에서** 수행. Lighthouse(Admin)는 워크트리 없음 — repo 자체(`Stoa/Stoa/`, 프로젝트명) 그대로 진행. *(이유: 룰 16/18 path 일관성. repo와 형제 워크트리는 같은 트리의 두 working copy지만 inode가 달라 — 한쪽에서 읽고 다른 쪽에서 쓰면 untracked drop·monitor catch 실패가 생긴다. 출근 직후부터 단일 path로 고정.)*

   **cwd basename = 자기 이름**. 현재 `pwd` 결과의 마지막 segment가 곧 자기 정체. `/Users/.../Stoa/Brandon/`이면 너는 Brandon, `/Users/.../Stoa/Stoa/`이면 너는 Admin. CLAUDE.md의 *Admin-narrative-heavy* 본문을 읽고 자기를 Admin으로 흡수하지 말 것 — 2026-05-14 Brandon identity 혼동 사고 직접 학습, 룰 24 land 자리.
...
5. 자기 폴더 `ClaudeTeam/<자신>/identity/`를 순서대로 읽는다 — `Identity.md` → `Bonds.md` → `Will.md`. (CLAUDE.md 룰 24 1단계 정합 — *명시적 Read*가 의무. step 3의 `cwd basename`이 폴더 이름.)
