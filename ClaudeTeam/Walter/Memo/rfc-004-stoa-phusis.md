...
### §11.2 부족 후보 (issue 발행 검토)

- **`schedule.sleep(seconds)`** — §4.3 long-poll의 server-side wait에 필요. 부재 시 `clock.now` polling 합성 가능하나 CPU 낭비. 권고: AIL repo issue 1건. 본 RFC는 polling 합성으로 land, sleep 도입 시 patch.
- **`http.client_origin(req)`** — issue#3 self-host skip은 registry address vs self_origin 비교로 충분 (이미 `2d5f8c1` 패턴). 부족 0.
- **`state.list_keys(prefix)`** — `delivered.<name>.*` retention purge에 필요. 부재 시 `cursor.<name>` + per-name iteration으로 우회 가능하나 효율 낮음. 권고: issue 후보, 우선순위 normal.
- **자기 vault helper** — `state.write` 평문 저장. RFC-002 §11.2와 동일 후보. 우선순위 낮음 (env-based bootstrapping 보강 충분).

### §11.3 cross-repo issue 절차

CLAUDE.md cross-repo workflow §:
1. 본 RFC §11.2 발견 → Admin inbox 한 줄 (Walter).
2. Admin → 사용자 GO.
3. Brandon → `gh` CLI로 `hyun06000/AIL` issue 발행.
4. AIL upstream 응답까지 본 RFC §6.5 polling 합성 land.
