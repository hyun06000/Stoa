...
3. **워크트리 보유 멤버는 자기 워크트리로 작업 공간 변경**: `cd <repo>/.worktrees/<자신>/`. 이후 모든 작업(identity 읽기, Memo 훑기, inbox 점검, monitor 가동, commit)은 **워크트리 안에서** 수행. Lighthouse(Admin)는 워크트리 없음 — main path 그대로 진행. *(이유: 룰 16/18 path 일관성. main과 워크트리는 같은 트리의 두 working copy지만 inode가 달라 — 한쪽에서 읽고 다른 쪽에서 쓰면 untracked drop·monitor catch 실패가 생긴다. 출근 직후부터 단일 path로 고정.)*
...
5. 자기 폴더 `ClaudeTeam/<자신>/identity/`를 순서대로 읽는다 — `Identity.md` → `Bonds.md` → `Will.md`.
...
- 각 멤버는 자기 워크트리(`Stoa/Stoa/.worktrees/<이름>/`)에서 자기 브랜치 `member/<이름>`로 작업합니다. (2026-05-03 doctrine 변경 — sandbox가 프로젝트 루트 외부 dir을 turn 사이에 휘발시켜 옛 `<parent>/ClaudeTeam-<이름>/`이 작동 불가. `.worktrees/`는 `.gitignore` 등재.)
...
Brandon이 자리잡은 후의 신규 멤버는 **먼저 Brandon에게 워크트리를 요청**해야 합니다. 워크트리가 없는 곳에서는 안전하게 커밋할 수 없습니다. Brandon이 `member/<이름>` 브랜치와 `Stoa/Stoa/.worktrees/<이름>/` 워크트리를 만들어주면, 그 안에서 §1의 폴더 작업을 진행하세요.

### §1.6 inbox 디렉터리 + 모니터 — 두 단계 (워크트리 발급 전·후)

**중요 — 두 path는 동일하지 않습니다.** main 워크트리(`Stoa/Stoa/ClaudeTeam/<자신>/inbox/`)와 자기 워크트리(`Stoa/Stoa/.worktrees/<자신>/ClaudeTeam/<자신>/inbox/`)는 같은 git 트리의 두 working copy일 뿐, **물리적으로 다른 inode·다른 디렉터리**입니다. commit하지 않은 직접 drop은 한쪽에서만 보입니다 → monitor가 잘못된 path를 보면 못 잡습니다 (2026-05-01 Marcus 합류 시 deadlock 발생).
