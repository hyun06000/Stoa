...
import pytest
from stoa_client.core import canonical_letter, sign_envelope, _esc, keygen, KeyExistsError
...
class TestSignEnvelope:
    def test_no_key_noop(self, tmp_path, monkeypatch):
        monkeypatch.setattr("stoa_client.core.keys_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "stoa_client.core.key_path",
...
class TestKeyExistsGuard:
    def test_keygen_refuses_overwrite(self, tmp_path, monkeypatch):
        ...
        keygen("alice", "", dry_run=True)
        with pytest.raises(KeyExistsError):
            keygen("alice", "", dry_run=True)

    def test_keygen_force_overwrites(self, tmp_path, monkeypatch):
        monkeypatch.setattr("stoa_client.core.keys_dir", lambda: tmp_path)
        monkeypatch.setattr(
            "stoa_client.core.key_path",
            lambda name: (tmp_path / f"{name}.key", tmp_path / f"{name}.pub"),
        )
        first = keygen("bob", "", dry_run=True)
        second = keygen("bob", "", dry_run=True, force=True)
        assert first["pk_hex"] != second["pk_hex"]
